...
# Type alias for the canonical structure dict: {chain → {residue_key → [atom_dict]}}
StructureDict = Dict[str, Dict[str, List[Dict[str, Any]]]]

# Standard PDB Atom Order for sorting (Critical for VMD Ribbons & Sidechains)
STANDARD_ATOM_ORDER = [
    "N", "CA", "C", "O", "CB", "CG", "CG1", "CG2", 
    "OG", "OG1", "SG", "CD", "CD1", "CD2", "SD", 
    "ND1", "ND2", "OD1", "OD2", "CE", "CE1", "CE2", "CE3", 
    "NE", "NE1", "NE2", "OE1", "OE2", "CZ", "CZ2", "CZ3", 
    "CH2", "NH1", "NH2", "OH", "OXT"
]

def parse_cif(file_path: str) -> StructureDict:
    """
    ...
    seen_atoms = set() # Add this tracker to handle altloc filtering
    ...
    while i < len(lines):
        ...
        if parsing_atoms:
            ...
            # --- ALTLOC FILTER ---
            atom_name = atom_dict.get('label_atom_id', atom_dict.get('auth_atom_id', 'X'))
            unique_atom_id = f"{chain_id}_{residue_key}_{atom_name}"
            
            if unique_atom_id in seen_atoms:
                i += 1
                continue
                
            seen_atoms.add(unique_atom_id)
    ...
    if not result:
        warnings.warn(
            f"No _atom_site loop found in {file_path}. "
            "The CIF file may be missing coordinate data or may use "
            "a non-standard loop category.",
            stacklevel=2,
        )
    return result


def write_cif(structure_dict: StructureDict, output_path: str, data_block_name: str = "STRUCTURE") -> None:
    """
    ...
    atom_order_priority = {name: i for i, name in enumerate(STANDARD_ATOM_ORDER)}
        ...
        for res_key in sorted_res_keys:
            ...
            for atom in atom_list:
                ...
                if "type_symbol" in atom:
                    ...
                    row["type_symbol"] = elem_clean[0] if elem_clean else "X"
...
def write_pdb(structure_dict: StructureDict, output_path: str) -> None:
    """
...
def parse_pdb(file_path: str) -> StructureDict:
    """
    ...
    # The main data structure
    structure_data = defaultdict(lambda: defaultdict(list))
    # Add a tracker for seen atoms per residue
    seen_atoms = set()
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            # Warn once if MODEL records detected (multi-model PDB)
            if record_type == "MODEL" and not _model_warning_emitted:
                warnings.warn(
                    f"PDB file '{file_path}' contains MODEL records "
                    f"(NMR ensemble or MD trajectory). Only the first "
                    f"model will be parsed; subsequent models are ignored.",
                    stacklevel=2,
                )
            ...
            if record_type in ("ATOM", "HETATM"):
                ...
                # --- ALTLOC FILTER ---
                # Create a unique key for this specific atom in this specific residue
                unique_atom_id = f"{chain_id}_{residue_key}_{atom_name}"
                
                # If we have already seen this atom name for this residue, 
                # it is a secondary altloc. Ignore it.
                if unique_atom_id in seen_atoms:
                    continue
